...
public class AIMLProcessor {
    ...
    private static void categoryProcessor(Node n, ArrayList<Category> categories, String topic, String aimlFile, String language) throws TransformerException, IOException {
    ...
    private static String sraix(Node node, ParseState ps) {
        ...
        String result = null;//Sraix.sraix(ps.chatSession, evalResult, defaultResponse, hint, host, botid, null, limit);
        return result;
    ...
    private static String map(Node node, ParseState ps) {
        ...
        if (mapName == null) result = "<map>" + contents + "</map>"; // this is an OOB map tag (no attribute)
            ...
            AIMLMap map = ps.chatSession.bot.getMapMap().get(mapName);
    ...
    private static String set(Node node, ParseState ps) {                    // add pronoun check
        ...
        if (ps.chatSession.bot.getPronounSet().contains(predicateName)) {
    ...
    private static String bot(Node node, ParseState ps) {
        ...
        if (propertyName != null)
            result = ps.chatSession.bot.getProperties().get(propertyName).trim();
     ...
     */
    private static String system(Node node, ParseState ps) throws IOException {
 ...
 */
    public static String uniq(Node node, ParseState ps) {
        ...
        StringBuilder tupleListBuilder = new StringBuilder();
        for (Tuple tuple : tuples) {
            tupleListBuilder.insert(0, tuple.getName() + " ");
        }
        String tupleList = tupleListBuilder.toString();
    ...
    public static String select(Node node, ParseState ps) {
        ...
        for (Tuple tuple : tuples) {
            resultBuilder.insert(0, tuple.getName() + " ");
        }
