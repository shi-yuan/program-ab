...
public class AIMLSet extends HashSet<String> {
    ...
    public boolean contains(String s) {
        ...
        if (isExternal && MagicBooleans.enable_external_sets) {
            ...
            String response = null;//Sraix.sraix(null, query, "false", null, host, botid, null, "0");
    ...
    public  void writeAIMLSet () {
        ...
        try{
            // Create file
            FileWriter fstream = new FileWriter(bot.getSetsPath()+"/"+setName+".txt");
    ...
    public int readAIMLSet (Bot bot) {
        int cnt=0;
        if (MagicBooleans.trace_mode) System.out.println("Reading AIML Set "+bot.getSetsPath()+"/"+setName+".txt");
        try{
            // Open the file that is the first
            // command line parameter
            File file = new File(bot.getSetsPath()+"/"+setName+".txt");
            if (file.exists()) {
                FileInputStream fstream = new FileInputStream(bot.getSetsPath()+"/"+setName+".txt");
                // Get the object
                cnt = readAIMLSetFromInputStream(fstream, bot);
                fstream.close();
            }
            else System.out.println(bot.getSetsPath()+"/"+setName+".txt not found");
        }catch (Exception e){//Catch exception if any
            System.err.println("Error: " + e.getMessage());
        }
        return cnt;
